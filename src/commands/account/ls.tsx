...
function AccountsList({ accounts }: { accounts: Account[] }): React.ReactElement {
  ...
  const columns: Column<AccountRow>[] = [
    ...
    ...(hasOws ? [{ key: "owsWallet" as const, header: "OWS Wallet" }] : []),
  ]
    ...
    .action(async function (this: Command) {
      ...
      if (outputOpts.json) {
        ...
        const jsonAccounts = accounts.map((acc) => ({
          ...
          owsWalletName: acc.owsWalletName,
          owsApiKey: acc.owsApiKey ? "[REDACTED]" : null,
